...
router.get('/getAll', (req, res) => {
  db.getAllArticles((data) => res.send(data));
});
...
router.post('/write', isLogin, async (req, res) => {
  if (req.body.title && req.body.content) {
    ...
    const err = new Error('글 제목 또는 내용이 빠졋습니다.');
    throw err;
  }
});
...
router.get('/modify/:id', isLogin, async (req, res) => {
  const findArticle = await db.getArticles(req.params.id);
  if (findArticle) {
...
router.post('/modify/:id', isLogin, async (req, res) => {
  if (req.body.title && req.body.content) {
    const modifyArticle = await db.modifyArticle(req.params.id, req.body);
    if (modifyArticle) {
      res.redirect('/dbBoard');
    } else {
...
router.delete('/delete/:id', isLogin, async (req, res) => {
  if (req.params.id) {
    ...
    const err = Error('ID 파라미터 값이 업습니다.');
    err.statusCode = 404;
    throw err;
  }
});
